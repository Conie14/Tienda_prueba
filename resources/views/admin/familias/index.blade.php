    ...
    <div class="p-4 bg-white rounded-lg shadow-md">
        @if ($familias->count() > 0)
            <div class="mb-4 flex flex-col md:flex-row items-center justify-between">
                ...
                <div class="w-full md:w-auto flex items-center space-x-2">
                    <div id="familiasTable_filter" class="relative w-full md:w-64">
                        <form id="searchForm" action="{{ route('admin.familias.index') }}" method="GET">
                        ...
                        <tr>
                            <th scope="col" class="px-6 py-3 text-center">
                                <div class="flex items-center justify-center">
                                    <a href="{{ route('admin.familias.index', ['sort' => 'id_familia', 'direction' => request('sort') === 'id_familia' && request('direction') === 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'per_page' => request('per_page', 10)]) }}" class="flex items-center">
                                        ID
                                        @if(request('sort') === 'id_familia')
                            ...
                            <th scope="col" class="px-6 py-3 text-center">
                                <div class="flex items-center justify-center">
                                    <a href="{{ route('admin.familias.index', ['sort' => 'nombre', 'direction' => request('sort') === 'nombre' && request('direction') === 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'per_page' => request('per_page', 10)]) }}" class="flex items-center">
                                        Nombre
                        ...
                        @foreach ($familias as $familia)
                            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                                ...
                                <td class="px-6 py-4 text-center space-x-2">
                                    ...
                                    <a href="{{ route('admin.familias.edit', $familia->id_familia) }}" class="text-blue-600 hover:text-blue-900" aria-label="Editar {{ $familia->nombre }}">
                                    ...
                                    <form action="{{ route('admin.familias.destroy', $familia->id_familia) }}" method="POST" class="inline-block" id="delete-form-{{ $familia->id_familia }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="text-red-600 hover:text-red-900 delete-btn" data-id="{{ $familia->id_familia }}" aria-label="Eliminar {{ $familia->nombre }}">
            ...
            <div class="mt-4 flex flex-col md:flex-row items-center justify-between">
                <div class="text-sm text-gray-700 mb-4 md:mb-0">
                    Mostrando {{ $familias->firstItem() ?? 0 }} a {{ $familias->lastItem() ?? 0 }} de {{ $familias->total() }} entradas
                </div>
        ...
        @else
            <div class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
                ...
                <div>
                    <span class="font-medium">Alerta!</span> No existe ninguna familia registrada en el sistema.
                </div>
            </div>
        @endif
    </div>
