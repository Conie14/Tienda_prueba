    ...
    <div class="p-4 bg-white rounded-lg shadow-md">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table id="familiasTable"
                    ...
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-center cursor-pointer" data-sort="id_provedor">
                            ...
                            {{-- direccion --}}
                            <th scope="col" class="px-6 py-3 text-center cursor-pointer" data-sort="direccion">
                            ...
                            {{-- telefono --}}
                            <th scope="col" class="px-6 py-3 text-center cursor-pointer" data-sort="telefono">
                                <div class="flex item-center justify-center">
                                    Teléfono
                                    <span class="sort-icon ml-1"></span>
                                </div>
                            ...
                            {{-- email --}}
                            <th scope="col" class="px-6 py-3 text-center cursor-pointer" data-sort="email">
                                <div class="flex item-center justify-center">
                                    Email
                                    <span class="sort-icon ml-1"></span>
                                </div>
                        ...
                        @foreach ($provedors as $provedor)
                            <tr
                                ...
                                <td class="px-6 py-4 text-center space-x-2">
                                    ...
                                    <a href="{{ route('admin.provedores.edit', $provedor->id_provedor) }}"
                                        class="text-blue-600 hover:text-blue-900"
                                        aria-label="Editar  {{ $provedor->nombre }}">
                                        <i class="fas fa-edit text-lg"
                                            data-tooltip-target="tooltip-edit-{{ $provedor->id_provedor }}"
                                            data-tooltip-placement="top"></i>
                                        <div id="tooltip-edit-{{ $provedor->id_provedor }}" role="tooltip"
                                    ...
                                    <form action="{{ route('admin.provedores.destroy', $provedor->id_provedor) }}"
                                        method="POST" class="inline-block" id="delete-form-{{ $provedor->id_provedor }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="text-red-600 hover:text-red-900 delete-btn"
                                            data-id="{{ $provedor->id_provedor }}"
                                            aria-label="Eliminar  {{ $provedor->nombre }}">
                                            <i class="fas fa-trash text-lg"
                                                data-tooltip-target="tooltip-delete-{{ $provedor->id_provedor }}"
                                                data-tooltip-placement="top"></i>
                                            <div id="tooltip-delete-{{ $provedor->id_provedor }}" role="tooltip"
                                                class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip">
                                                Eliminar
                                                <div class="tooltip-arrow" data-popper-arrow></div>
                                            </div>
            ...
            <div class="mt-4 flex flex-col md:flex-row items-center justify-between">
                <div class="text-sm text-gray-700 mb-4 md:mb-0">
                    Mostrando <span id="showingStart">1</span> a <span id="showingEnd">10</span> de <span
                        id="totalEntries">{{ $provedors->count() }}</span> entradas
                </div>
        ...
        @else
            <div class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400"
                ...
                <div>
                    <span class="font-medium">Alerta!</span> No existe ningu provedor registrado en el sistema.
                </div>
            </div>
        @endif
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('exportExcel').addEventListener('click', function() {
                ...
                XLSX.utils.book_append_sheet(wb, ws, 'Provedors');

                // Guardar archivo
                XLSX.writeFile(wb, 'Provedors_' + new Date().toISOString().slice(0, 10) + '.xlsx');
            });
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Eliminado',
                                text: 'El provedor ha sido eliminada correctamente.',
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Cancelado',
                                text: 'El provedor no ha sido eliminada.',
                                showConfirmButton: false,
                                timer: 1500
                            });
                        }
